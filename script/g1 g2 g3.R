...
"Can we predict g3 based on g1 and g2 values only? The resulting model should
be worse than the model using all the variables. But we will still try."
...
data <- filter(data, G2 != 0) #13 students have 0 G2 grade
data <- filter(data, G3 != 0)

#Finding the correlations again
correlation.clean <- c(G1.G2 = cor(data$G1, data$G2),
                       G2.G3 = cor(data$G2, data$G3),
                       G1.G3 = cor(data$G1, data$G3))
...
"Next we will try to predict G3 using a linear regression model with G1 and
G2 as the input."
